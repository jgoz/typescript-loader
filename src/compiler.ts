...
export function compile (ctx: CompileContext): Promise<CompileResult> {
    ...
    return executeNode([tsc].concat(args)).then(res => {
        ...
        var outStream: ReadableStream = fs.createReadStream(path.join(ctx.outPath, outFile), { encoding: "utf8" });
        ...
        if (ctx.options.sourcemap) {
            var mapStream: ReadableStream = fs.createReadStream(path.join(ctx.outPath, outFile + ".map"), { encoding: "utf8" });
